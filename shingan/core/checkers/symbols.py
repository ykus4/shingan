...
def check(binary_path: Path) -> list[Finding]:
    ...
    suspicious = [s for s in debug_syms if any(
    ...
    if objc_classes:
        ...
        findings.append(Finding(
            ...
            evidence="\n".join(sample) + (f"\n… and {len(objc_classes) - 30} more" if len(objc_classes) > 30 else ""),
            recommendation=(
    ...
    swift_syms = [sym.name for sym in binary.symbols if sym.name.startswith("$s") or sym.name.startswith("_$s")]
    if swift_syms:
