...
SECRET_PATTERNS: list[tuple[str, str, re.Pattern]] = [
    ...
    ("aws_secret", "AWS Secret (candidate)",    re.compile(r"(?i)aws.{0,20}secret.{0,5}['\"]([A-Za-z0-9/+=]{40})")),
    ...
    ("generic_key","Generic 'key/secret/token'",re.compile(r"(?i)(api_key|apikey|secret_key|access_token|auth_token)\s*[=:]\s*['\"]?([A-Za-z0-9_\-]{16,})")),
    ("http_url",   "Plain HTTP URL (non-localhost)", re.compile(r"http://(?!localhost|127\.0\.0\.1|0\.0\.0\.0)[a-zA-Z0-9\-\.]+\.[a-zA-Z]{2,}")),
    ("endpoint",   "Hardcoded endpoint/path",   re.compile(r"/api/v\d+/[a-zA-Z0-9/_\-]{5,}")),
]
...
def check(binary_path: Path) -> list[Finding]:
    ...
    for line in strings:
        ...
        if len(s) >= ENTROPY_MIN_LEN and _shannon_entropy(s) >= ENTROPY_THRESHOLD:
            ...
            if not any(s.startswith(fp) for fp in ("https://", "http://", "com.", "org.", "eyJ")):
